...
# Balanced detection confidence - not too sensitive, not too strict
hands = mp_hands.Hands(
    max_num_hands=1, 
    min_detection_confidence=0.4,  # Higher confidence to avoid false positives
    min_tracking_confidence=0.4,   # Higher confidence to avoid false positives
...
def detect_prev_gesture(landmarks):
    """Detect pointing left gesture (index extended, thumb pointing left)"""
    # Get finger tip landmarks
    thumb_tip = landmarks[4]
    index_tip = landmarks[8]
    middle_tip = landmarks[12]
    ...
    # Index finger should be extended (check absolute difference)
    index_extended = abs(index_base.y - index_tip.y) > finger_threshold
    
    # Thumb should be pointing left (much more lenient check)
    # Just check if thumb is pointing left, don't require it to be fully extended
    thumb_left = (thumb_tip.x - thumb_base.x) < -0.01  # Small threshold for left direction
    
    # Other fingers should be closed (check absolute difference)
    ...
    # For previous: index extended + thumb pointing left + others closed
    return index_extended and thumb_left and middle_closed and ring_closed and pinky_closed

def validate_hand(landmarks):
    """Validate if the detected landmarks form a reasonable hand shape"""
    # Check if hand is reasonably sized (not too small = not too far)
    wrist = landmarks[0]
    middle_finger_tip = landmarks[12]
    hand_size = np.sqrt((wrist.x - middle_finger_tip.x)**2 + (wrist.y - middle_finger_tip.y)**2)
    
    # Hand should not be too small (too far) or too large (too close)
    if hand_size < 0.05 or hand_size > 0.8:
        return False, "Hand size invalid"
    
    # Check if hand is roughly in the center area of the frame
    wrist_x, wrist_y = wrist.x, wrist.y
    if wrist_x < 0.05 or wrist_x > 0.95 or wrist_y < 0.05 or wrist_y > 0.95:
        return False, "Hand position invalid"
    
    # Check if landmarks form a reasonable hand structure
    # Distance between wrist and palm should be reasonable
    palm = landmarks[9]  # Middle finger base
    wrist_palm_dist = np.sqrt((wrist.x - palm.x)**2 + (wrist.y - palm.y)**2)
    
    if wrist_palm_dist < 0.02 or wrist_palm_dist > 0.3:
        return False, "Hand structure invalid"
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            # Validate hand first
            is_valid_hand, validation_msg = validate_hand(hand_landmarks.landmark)
            
            if not is_valid_hand:
                # Skip this detection if hand is not valid
                continue
            ...
            cv2.putText(frame, f"Valid: {is_valid_hand}", (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"Palm: {is_palm}", (10, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"Next: {is_next}", (10, 120), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"Prev: {is_prev}", (10, 150), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            ...
            cv2.putText(frame, f"Thumb: {thumb_diff:.3f}", (10, 180), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
            cv2.putText(frame, f"Index: {index_diff:.3f}", (10, 200), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
            cv2.putText(frame, f"Middle: {middle_diff:.3f}", (10, 220), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
